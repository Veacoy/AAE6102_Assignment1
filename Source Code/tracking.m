...
multiCorrelators = -0.5:0.1:0.5;
multicorr_res = zeros(length(find([channel.PRN]~=0)),length(multiCorrelators));
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:codePeriods
            ...
            %% Multicorrelator
            if loopCnt == 10000
                for i = 1:length(multiCorrelators)
                    correlator = multiCorrelators(i);
                    tcode       = (remCodePhase+correlator) : ...
                        codePhaseStep : ...
                        ((blksize-1)*codePhaseStep+remCodePhase+correlator);
                    tcode2      = ceil(tcode) + 1;
                    ...
                    I_C = sum(cCode.*iBasebandSignal);
                    Q_C = sum(cCode.*qBasebandSignal);

                    multicorr_res(channelNr,i) = sqrt(I_C^2 + Q_C^2);



                end
                if channelNr == length(find([channel.PRN]~=0))
                    figure;
                    hold on;
         
                    plot(multiCorrelators,multicorr_res,LineWidth=1.5);
