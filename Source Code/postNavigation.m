...
%% Find pseudoranges ======================================================
    ...
    % Save transmitTime
    navSolutions.rawD(:, currMeasNr) = calculateDopplers(trackResults,currMeasSample,activeChnList,settings);
...
%% Find satellites positions and clocks corrections =======================
    ...
    [satPositions, satClkCorr, satVelocitys, satClkCorrRat] = satpos(transmitTime(activeChnList), ...
...
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        % Correct doppler for SV clock error
        clkCorrRawD = navSolutions.rawD(activeChnList, currMeasNr)' - 1575.42e6*satClkCorrRat;
        ...
        [xyzdt,navSolutions.el(activeChnList, currMeasNr), ...
               ...
               navSolutions.DOP(:, currMeasNr)] =...
                       wlsPos(satPositions, clkCorrRawP, settings);

        % Calculate receiver velocity
        velddt = wlsVel(satPositions, clkCorrRawP, settings, satVelocitys, clkCorrRawD, satClkCorrRat);
        ...
        navSolutions.velX(currMeasNr) = velddt(1);
        navSolutions.velY(currMeasNr) = velddt(2);
        navSolutions.velZ(currMeasNr) = velddt(3);
...
%% Coordinate conversion ==================================================
    ...
    if currMeasNr == measNrSum
        figure;
        geobasemap satellite
        hold on;
        geoplot(navSolutions.latitude, navSolutions.longitude, 'r.');
        geoplot(22.3198722, 114.209101777778,'b*')
        hold off;

        figure;
        hold on;
        plot(navSolutions.velX,'r');
        plot(navSolutions.velY,'g');
        plot(navSolutions.velZ,'blue');
        hold off;
    end
    mean(navSolutions.latitude)
    mean(navSolutions.longitude)
