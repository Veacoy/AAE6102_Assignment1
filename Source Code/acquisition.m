...
%% Look for correlation peaks in the results ==============================
    ...
    % 假设你已拥有 result (41 x 26000)
    
    % 构造 X 和 Y 坐标轴：
    [m, n] = size(results);
    code_offset = 0:n-1;                % 横轴：码偏移索引（或实际单位）
    doppler_freq = linspace(0,m,m);  % 纵轴：多普勒频率（Hz）
    
    if(PRN==16)
        % 创建网格
        [X, Y] = meshgrid(code_offset, doppler_freq);
        ...
        title('GNSS Acquisition Correlation Peak Map (3D) PRN 16');
    end
    if(PRN==1)
        % 创建网格
        [X, Y] = meshgrid(code_offset, doppler_freq);
        ...
        ylabel('Doppler Frequency (Hz)');
        zlabel('Correlation Magnitude');
        title('GNSS Acquisition Correlation Peak Map (3D) PRN 1');
    end
